...
def transaction_pdf(transaction: models.Transaction, settings: models.Settings | None) -> bytes:
    ...
    company = settings.company_name if settings else "SKY ARIANA GROUP OF COMPANIES"
    address = settings.address if settings else "Kandahar, Afghanistan"
    footer = settings.receipt_footer if settings else "Official receipt generated by SKY ARIANA GROUP OF COMPANIES.2026"
    lines = [
        "q 0.058 0.42 0.86 rg 48 734 64 64 re f Q",
        "BT /F2 18 Tf 1 1 1 rg 67 757 Td (SA) Tj ET",
...
def list_customers(
    ...
    if entity_type and not isinstance(entity_type, type(Query(None))):
